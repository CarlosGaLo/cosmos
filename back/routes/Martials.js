...
router.get("/", async (req, res) => {
  const martial = await Martial.find();
  res.json(martial);
});
...
router.get("/:name", async (req, res) => {
  try {
    ...
    const martial = await Martial.findOne({ name: name });
    if (!martial) {
      return res.status(404).json({ message: "Martial no encontrado" });
    }
    res.json(martial);
  } catch (error) {
    res.status(500).json({ message: error.message });
...
router.put("/:name", async (req, res) => {
  try {
    ...
    // Update properties
    martial.name = newName || martial.name;
    martial.resume = newResume || martial.resume;
    martial.xp = newXP || martial.resume;
    martial.description = newDescription || martial.resume;
    martial.requisites = newRequisites || martial.resume;

    // Guardar el artículo actualizado
    const updatedFeat = await martial.save();
...
router.delete("/:name", async (req, res) => {
  try {
    ...
    // Buscar y eliminar el artículo por su nombre
    const deletedFeat = await Martial.findOneAndDelete({ name: name });

    if (!deletedFeat) {
      return res.status(404).json({ message: "Martial no encontrado" });
    }

    res.json({ message: "Martial eliminado exitosamente" });
  } catch (error) {
    res.status(500).json({ message: error.message });
  }
});
