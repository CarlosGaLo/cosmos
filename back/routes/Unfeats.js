...
router.get("/", async (req, res) => {
  const unfeat = await Unfeat.find();
  res.json(unfeat);
});
...
router.get("/:name", async (req, res) => {
  try {
    ...
    const unfeat = await Unfeat.findOne({ name: name });
    if (!unfeat) {
...
router.delete("/:name", async (req, res) => {
  try {
    ...
    // Buscar y eliminar el artículo por su nombre
    const deletedFeat = await Unfeat.findOneAndDelete({ name: name });

    if (!deletedFeat) {
      return res.status(404).json({ message: "Unfeat no encontrado" });
    }

    res.json({ message: "Unfeat eliminado exitosamente" });
  } catch (error) {
    res.status(500).json({ message: error.message });
  }
});
