<template>
  <div class="hc-theme p-4 space-y-6">
    ...
    <header class="hc-card flex items-center justify-between">
      <h1 class="text-2xl font-bold">Calculador de Iniciativa — Anima: Beyond Fantasy</h1>
      <div class="space-x-2">
          ...
          <tbody>
            <tr
              ...
              :class="{
                ...
                'edge-lose': dominanceMap.get(c.id)?.dominatedBy.length && !dominanceMap.get(c.id)?.dominates.length,
              }"
            >
              ...
              <td class="py-2 pr-4 text-right">
                <input v-model.number="c.base" type="number" class="hc-input w-24 text-right" />
              </td>
              <td class="py-2 pr-4 text-right">
                <input v-model.number="c.mod" type="number" class="hc-input w-24 text-right" />
              </td>
              <td class="py-2 pr-4 text-right font-semibold tabular-nums">{{ c.final }}</td>
              <td class="py-2 pr-4">
                <div class="flex flex-col gap-1">
                  <template v-if="dominanceMap.get(c.id)?.dominates.length">
                    <span
                      ...
                      :title="`${c.nombre} supera a ${findName(d.otherId)} por ${d.diff}`"
                  ...
                  <template v-if="dominanceMap.get(c.id)?.dominatedBy.length">
                    <span
                      ...
                      :title="`${findName(d.otherId)} supera a ${c.nombre} por ${d.diff}`"
              ...
              <td class="py-2 pr-4">
                <div class="space-y-1 text-xs">
                  <div v-if="c.lastRoll">
                    <div><span class="font-semibold">Cadena:</span> {{ c.lastRoll.rolls.join(' → ') }}</div>
                    <div>
                      <span class="font-semibold">Suma:</span> +{{ c.lastRoll.add }}
                      <span v-if="c.lastRoll.sub"> / <span class="font-semibold">Resta:</span> -{{ c.lastRoll.sub }}</span>
                    </div>
                  </div>
              ...
              <td class="py-2 pr-4">
                <div class="flex flex-wrap gap-2">
                  <button class="hc-btn-sm" @click="rollForCharacter(c)">Tirar</button>
                  <button class="hc-btn-sm" @click="removeCharacter(c.id)">Eliminar</button>
                </div>
              </td>
            </tr>
          </tbody>
    ...
    <section class="hc-card space-y-2">
      ...
      <div v-if="logs.length === 0" class="text-sm opacity-80">Sin registros todavía.</div>
      <ul v-else class="space-y-3 max-h-96 overflow-auto pr-2">
        <li v-for="(entry, i) in logs" :key="i" class="hc-log-item">
          <div class="text-xs opacity-80 mb-1">{{ formatDate(entry.time) }}</div>
          <div class="text-sm">
            <div class="font-semibold mb-1">{{ entry.title }}</div>
            <ul class="list-disc ml-5 space-y-1">
              <li v-for="(line, j) in entry.lines" :key="j" class="break-words">{{ line }}</li>
            </ul>
          </div>
        </li>
      </ul>
    ...
    <section class="hc-card">
      <button class="hc-accordion" @click="showAdder = !showAdder" :aria-expanded="showAdder.toString()">
        <span class="font-semibold">Añadir personaje</span>
        <span class="chev" :class="{ open: showAdder }">▾</span>
      </button>
      <transition name="fade-slide">
        <form v-if="showAdder" @submit.prevent="addCharacter" class="grid grid-cols-1 md:grid-cols-6 gap-3 items-end mt-3">
          <div class="md:col-span-2">
            <label class="block text-sm font-medium">Nombre</label>
            <input v-model.trim="newChar.nombre" type="text" class="hc-input w-full" placeholder="Nombre" required />
          </div>
          <div>
            <label class="block text-sm font-medium">Base</label>
            <input v-model.number="newChar.base" type="number" class="hc-input w-full" placeholder="0" required />
          </div>
          <div>
            <label class="block text-sm font-medium">Mod</label>
            <input v-model.number="newChar.mod" type="number" class="hc-input w-full" placeholder="0" />
          </div>
          <div class="md:col-span-2 flex gap-2">
            <button type="submit" class="hc-btn">Añadir</button>
            <button type="button" class="hc-btn" @click="addAndRoll">Añadir + Tirar</button>
          </div>
        </form>
      </transition>
    </section>
...
function rollForCharacter(c) {
  ...
  c.lastRoll = last
  computeFinal(c)
  const line = `${c.nombre}: Base ${c.base} + Mod ${c.mod} + [${last.rolls.join(' → ')}] = ${c.final}`
...
function rollAll() {
  ...
  characters.forEach(c => {
    ...
    c.lastRoll = last
    computeFinal(c)
    lines.push(`${c.nombre}: Base ${c.base} + Mod ${c.mod} + [${last.rolls.join(' → ')}] = ${c.final}`)
  })
...
.hc-card {
  background: radial-gradient(120% 120% at 0% 0%, var(--hc-surface) 0%, var(--hc-surface-2) 100%);
...
.hc-btn, .hc-btn-sm {
  background: linear-gradient(180deg, var(--hc-accent) 0%, var(--hc-accent-2) 100%);
  color: #071018;
  border: none;
  border-radius: 12px;
  padding: 10px 14px;
...
.hc-btn:hover, .hc-btn-sm:hover { filter: brightness(1.05); }
.hc-btn:active, .hc-btn-sm:active { transform: translateY(1px); }
.hc-btn-sm { padding: 6px 10px; font-size: 12px; border-radius: 10px; }
...
.hc-input:focus { box-shadow: 0 0 0 3px rgba(125,211,252,0.25); }
...
.hc-table tbody tr { background: rgba(17, 24, 39, 0.6); }
.hc-table tbody tr:hover { background: rgba(17, 24, 39, 0.85); }
.hc-table tbody td { border-top: 1px solid rgba(125,211,252,0.18); border-bottom: 1px solid rgba(125,211,252,0.18); }

.tabular-nums { font-variant-numeric: tabular-nums; }

/* Dominance highlighting */
.edge-win { outline: 2px solid rgba(52, 211, 153, 0.35); }
.edge-lose { outline: 2px solid rgba(251, 113, 133, 0.35); }
...
.badge-win { background: rgba(52, 211, 153, 0.15); color: var(--hc-win); border: 1px solid rgba(52,211,153,0.35); }
.badge-lose { background: rgba(251, 113, 133, 0.12); color: var(--hc-lose); border: 1px solid rgba(251,113,133,0.35); }
...
.hc-accordion .chev { transition: transform 0.2s ease; }
.hc-accordion .chev.open { transform: rotate(180deg); }
